"""OpenSearch index mappings for sourceData knowledge bases."""
...
def rag_knowledge_index_body() -> dict[str, Any]:
    """
    Mapping for sourceData/data/rag_knowledge.jsonl — keyword + semantic (hybrid-ready).
...
def rag_department_rules_index_body() -> dict[str, Any]:
    """Keyword mapping for sourceData/data/rag_department_rules.jsonl."""
    return {
...
def triage_templates_index_body() -> dict[str, Any]:
    """Keyword-only mapping (no vectors) for sourceData/data/triage_templates.jsonl."""
    return {
        "settings": {"index": {"number_of_shards": 1, "number_of_replicas": 0}},
        "mappings": {
            "properties": {
...
def disease_kb_index_body() -> dict[str, Any]:
    """Keyword mapping for sourceData/data/disease_kb.jsonl — alias / canonical → departments."""
    return {
        "settings": {"index": {"number_of_shards": 1, "number_of_replicas": 0}},
        "mappings": {
            "properties": {
