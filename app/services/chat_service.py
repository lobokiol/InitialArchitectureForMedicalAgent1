...
def is_graph_ready() -> bool:
    """Public readiness probe for /ready without touching private globals."""
...
def _read_checkpoint_state(thread_id: str, user_id: str) -> AppState | None:
    ...
    try:
        snap = _app.get_state(run_config)
...
def _awaiting_flags(state: AppState) -> tuple[bool, bool, list, list, bool, Any]:
    ds = state.dept_state
    ...
    return awaiting_dept, awaiting_clarify, dept_choices, clarify_choices, multi_select, cs
...
def _build_chat_result(
    *,
    ...
    """Assemble ChatResponse-shaped dict for success and timeout paths."""
    if timed_out or state is None:
        return {
            "user_id": user_id,
            "thread_id": thread_id,
            "reply": reply,
    ...
    awaiting_dept, awaiting_clarify, dept_choices, clarify_choices, multi_select, cs = _awaiting_flags(
        state
    )
    conf = state.dept_confidence_result
    intent_dict = (
        state.intent_result.model_dump() if isinstance(state.intent_result, IntentResult) else None
    )
...
def chat_once(
    ...
    node_trace: list[str] = []
    for chunk in _app.stream(inputs, config=run_config, stream_mode="updates"):
